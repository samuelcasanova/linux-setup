...
sudo cp ~/git/setups/linux-setup-private/wireguard/wg1.conf /etc/wireguard/ || if [ ${?} -gt 0 ]; then exit 1; fi
...
sudo cp ~/git/setups/linux-setup/install-software-scripts/support-files/wireguard.service /etc/systemd/system/wireguard.service || if [ ${?} -gt 0 ]; then exit 1; fi
