...
wget -qO- "https://raw.githubusercontent.com/nvm-sh/nvm/v${RPM_LOCAL_PATH}/install.sh" | bash || if [ ${?} -gt 0 ]; then exit 1; fi

sudo apt install "${RPM_LOCAL_PATH}" || if [ ${?} -gt 0 ]; then exit 1; fi
